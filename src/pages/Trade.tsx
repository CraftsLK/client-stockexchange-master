...
export function Trade() {
  ...
  const [currencyCodes, setCurrencyCodes] = useState(["USD","GBP","LKR"]);
  ...
  useEffect(() => {
    ...
    if (!currCompany) {
      ...
      setPriceInUSD(currCompany["price"]);
      setSelectedCurrency(currCompany["currency"]);
      setCompanyCode(currCompany["symbol"]);
      setPricePerShare(currCompany["price"]);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // };
    // fetch(
    //   "http://localhost:8081/currency/get-all-curr-code",
    //   requestOptions
    // )
  ...
  useEffect(() => {
    ...
    //   };
    //   fetch(
    //     `http://localhost:8081/currency/exchange-rate?from=USD&to=${selectedCurrency}`,
    //     requestOptions
    //   )
    //     .then((response) => response.json())
    //     .then((data) => {
  ...
  const handleSubmit = (event: any) => {
    ...
    fetch("http://localhost:8081/trade/save", requestOptions)
      .then((res) => {
        setOpenDialog(true);
      })
